...
namespace RemoteCapture.Lib.WindowsRuntimeHelpers
{
    internal static class CoreMessagingHelper
    {
        ...
        static extern uint CreateDispatcherQueueController(DispatcherQueueOptions options, out nint dispatcherQueueController);
        ...
        public static DispatcherQueueController CreateDispatcherQueueControllerForCurrentThread()
        {
            ...
            uint hr = CreateDispatcherQueueController(options, out nint controllerPointer);
            if (hr == 0)
            {
