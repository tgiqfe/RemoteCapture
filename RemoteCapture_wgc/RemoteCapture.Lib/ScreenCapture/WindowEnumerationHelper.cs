...
namespace RemoteCapture.Lib.ScreenCapture
{
    public static class WindowEnumerationHelper
    {
        ...
        static extern nint GetAncestor(nint hwnd, GetAncestorFlags flags);
        ...
        static nint GetWindowLongPtr(nint hWnd, int nIndex)
        {
            if (nint.Size == 8)
        ...
        static extern int DwmGetWindowAttribute(nint hwnd, DWMWINDOWATTRIBUTE dwAttribute, out bool pvAttribute, int cbAttribute);

        public static bool IsWindowValidForCapture(nint hwnd)
        {
            if (hwnd == 0) return false;
            if (hwnd == GetShellWindow()) return false;
            if (!IsWindowVisible(hwnd)) return false;
            if (GetAncestor(hwnd, GetAncestorFlags.GetRoot) != hwnd) return false;

            var style = (WindowStyles)(uint)GetWindowLongPtr(hwnd, (int)GWL.GWL_STYLE).ToInt64();
            if (style.HasFlag(WindowStyles.WS_DISABLED)) return false;
